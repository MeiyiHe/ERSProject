...
#run the montreal aligner (used in genLib_m)
...
#get the phonemes from the dictionary (used in addTags)
...
#change unicode to ascii (mainly used in addTags)
...
#add tags (used in genLib_m)
def addTags( input_file, input_dict, output_file, aligner ):
...
def addTag(  input_file, output_file , aligner):
...
def preprocess(filename, preprocessed_script, aligner):

	#add tags
	addTag(filename, preprocessed_script, aligner)
...
def cover_phon(tag_script, inputDir, cost):
